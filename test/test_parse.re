...
let string_of_file = filename => {
  let path = Filename.concat(data_dir, filename);
  let chan = open_in(path);
  try({
    let length = in_channel_length(chan);
...
let test_parse_album_search = () => {
  let results =
    string_of_file("album_search.json") |> Album_j.search_wrapper_of_string;
          ...
          items: [
            {
              ...
              external_urls: [
                (
                  "spotify",
                  "https://open.spotify.com/album/5v4vDmq8pNO3TbegZmK5bL",
                ),
              ],
...
let test_parse_artist_search = () => {
  let results =
    string_of_file("artist_search.json") |> Artist_j.search_wrapper_of_string;
          ...
          items: [
            {
              ...
              followers: Followers_j.{href: None, total: 30},
              genres: [],
...
let test_parse_track_search = () => {
  let results =
    string_of_file("track_search.json") |> Track_j.search_wrapper_of_string;
          ...
          items: [
            {
              album:
                Album_j.{
                  ...
                  external_urls: [
                    (
                      "spotify",
                      "https://open.spotify.com/album/1eMrRFFbPPC2zgKGojsVxX",
                    ),
                  ],
                ...
                Artist_j.[
                  {
                    external_urls: [
                      (
                        "spotify",
                        "https://open.spotify.com/artist/14u4KXVp0iXQil79EpxXGc",
                      ),
                    ],
              ...
              duration_ms: 261346,
              explicit: false,
              external_ids: [("isrc", "UK2D51400006")],
              external_urls: [
                (
                  "spotify",
                  "https://open.spotify.com/track/7FPxCA9KaWXV18r1Za878d",
                ),
              ],
